...
void setupUDP() {
	...
	while (true) {
		...
		if (query.type == 0) {
			...
			cout << "The server A has sent a country list to Main Server"
					<< endl;
		} else {
			// find
			cout
					<< "The server A has received request for finding possible friends of User "
			...
			if (id == USERNOTFOUND) {
				...
				cout << "The server A has sent \"User " << query.id
						<< " not found\" to Main Server" << endl;
			} else if (id >= 0) {
				sprintf(msg, "%d", id);
				cout
						<< "The server A is searching possible friends for User<userID>"
